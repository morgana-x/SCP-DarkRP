...
namespace DarkRP.Modules.Players
{
    ...
    public class Job : BaseModule<JobConfig>
    {
      ...
      //  public static JobConfig Config { get { return SCPRP.Singleton.Config.JobConfig; } }
        ...
        {
            if (!Singleton.PlayerRoles.TryGetValue(player, out var oldjob))
                oldjob = Singleton.Config.DefaultJob;

            var eArgs = new Events.Arguments.Player.JobChangingEventArgs(player, oldjob, role, true);
            ...
            player.CustomInfo = GetColouredJobName(role);
        ...
        {
            if (Singleton == null || !Singleton.Config.Jobs.ContainsKey(job))
                return null;
